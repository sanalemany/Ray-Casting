...
function drawMenu() {
  ...
  menuScale = lerp(menuScale, 1, 2); // Escalado gradual del menú
  push();
  scale(menuScale);
  ...
  menuOptions.forEach((option, i) => {
    ...
    if (images[option]) {
      imageMode(CENTER);
      image(images[option], 0, 0, 150, 150);
    }
    pop();
  });
...
function drawInteractiveCircle(circle, lightX, lightY) {
  ...
  let brightness = map(d, 0, 300, maxBrightness, 0);
  brightness = constrain(brightness, 0, maxBrightness);

  for (let i = circle.r * 2; i > 0; i -= 10) {
